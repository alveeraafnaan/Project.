...
function readDb() {
  try {
    if (!fs.existsSync(dbPath)) {
      // Initialize with default database template from the repository
      const templatePath = path.join(__dirname, 'database.json');
      if (fs.existsSync(templatePath) && dbPath !== templatePath) {
        fs.copyFileSync(templatePath, dbPath);
      } else {
        const defaultData = { users: [], rooms: [], guests: [], payments: [], inventory: [], logs: [] };
